...
/**
 * Rule to detect empty lines or lines filled with whitespaces (spaces, tab feeds, etc.) 
 * 
 * @author xcoulon
 *
 * @see Character#isWhitespace(char)
 */
public class WhitespaceLinePartitionRule implements IScannablePartitionRule {

private final static String contentType = IDocument.DEFAULT_CONTENT_TYPE;
	 ...
	 */
	public WhitespaceLinePartitionRule() {
		super();
	...
	protected IToken doEvaluate(final ICharacterScanner scanner, final boolean resume) {
		final IToken result = readWhitespaceLine(scanner);
	 ...
	 */
	protected IToken readWhitespaceLine(final ICharacterScanner scanner) {
		...
		// if the scanner reached hit EOF
		if(c == ICharacterScanner.EOF) {
			return Token.EOF;
		}
		// otherwise, unread because the line was not empty.
		while(count > 0) {
			count--;
			scanner.unread();
		}
	...
	public static boolean isWhitespace(int codePoint) {
		return (((1 << Character.SPACE_SEPARATOR) >> Character.getType(codePoint)) & 1) != 0;
	}
